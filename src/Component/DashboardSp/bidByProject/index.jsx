...
class index extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      "https://shsbackend.azurewebsites.net/api/services/app/BidAppservice/GetNewBidByProject?ProjectId=" +
        id,
      {
        headers: {
                  ...
                  <div class="col-md-9 main-style-back mt-3">
                        ...
                        <div class="col-md-6 col-8 top-blue-para-right">
                          <p class="top-blue-para-right-p">
                            ...
                            <span class="top-blue-para-right-p-span">
                              <TimeAgo date={this.state.result.creationTime} />
                            </span>
                          </p>
                        </div>
                      </div>
                    ...
                    >
                      <Link
                        ...
                        to={`/bidDetails/${key.id}/${this.state.projectTitle}`}
                        style={{
                          paddingTop: "10px",
                          marginTop: " 0px",
                          textDecoration: "none",
  ...
  render() {
    ...
    // }
    const { result, isDataAvalaible } = this.state;
    const show =
      result.length === 0 ? (
        isDataAvalaible === false ? (
          <Loader />
        ) : (
          <h5 className="text-center pt-5">No Bid Availabe</h5>
        )
      ) : (
        this.state.result
      );
